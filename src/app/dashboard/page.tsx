...
export default async function DashboardPage() {
  ...
  const [profile, recentTrips, allTrips] = await Promise.all([
    ...
    prisma.trip.findMany({
      where: { userId },
      select: { days: true, destination: true },
    }),
  ]);
      ...
      <main className="min-h-screen bg-paper">
        <div className="max-w-5xl mx-auto px-6 md:px-8 pt-28 pb-20">
          ...
          <div className="bg-ink text-paper p-8 md:p-10 mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-8">
            ...
            <div className="flex-shrink-0">
              {availableCredits === 0 ? (
                <Link
                  href="/pricing"
                  className="inline-flex items-center gap-3 bg-burnt-orange text-white micro-copy px-8 py-4
                             hover:bg-burnt-orange/90 transition-colors duration-300"
                >
                ...
                <div className="inline-flex items-center gap-2 border border-emerald-accent/40 px-6 py-3">
                  ...
                  <span className="micro-copy text-emerald-accent">
                    Ready to curate
                  </span>
                </div>
              )}
            ...
            {recentTrips.length === 0 ? (
              ...
              <div className="bg-paper-dark border border-ink/10 p-12 text-center">
                ...
                <p className="font-sans text-sm text-ink-light leading-relaxed mb-8 max-w-xs mx-auto">
                  Your curated itineraries will appear here once you plan your
                  first journey.
                </p>
                <Link
                  href="/"
                  className="inline-flex items-center gap-2 bg-ink text-paper micro-copy px-8 py-4
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {recentTrips.map((trip) => (
                  <div
                    key={trip.id}
                    className="bg-paper-dark border border-ink/10 p-6 flex flex-col gap-3
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-16">
            <Link
              href="/"
              className="bg-burnt-orange text-white micro-copy px-8 py-5 text-center
                         hover:bg-burnt-orange/90 transition-colors duration-300 block"
            >
              Plan a New Journey &rarr;
            </Link>
            <Link
              href="/trips"
              className="border border-ink/20 micro-copy text-ink px-8 py-5 text-center
                         hover:bg-paper-dark transition-colors duration-300 block"
            >
